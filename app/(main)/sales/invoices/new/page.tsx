...
export default function NewCustomerInvoicePage() {
  ...
  useEffect(() => {
    ...
    fetchCustomersAndProducts(token);
  }, [router]);

  const fetchCustomersAndProducts = async (token: string) => {
    try {
      ...
      if (cRes.ok) {
        ...
        setCustomers(
          (cData.contacts || []).filter((c: any) => c.contact_type === 'Customer' || c.contact_type === 'Both')
        );
      }
      if (pRes.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErrors({ general: data.message || 'Failed to create customer invoice' });
      }
    } catch {
  ...
  return (
    <div className="max-w-[1000px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div className="mb-6">
        ...
        <h1 className="text-[24px] font-semibold text-[#111827]">New Customer Invoice</h1>
        <p className="text-[14px] text-[#667085] mt-1">Issue a receivable invoice directly to a customer</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-enterprise p-6 shadow-xs">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-[16px] font-semibold text-[#111827]">Invoiced Line Items</h2>
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-enterprise p-6 shadow-xs">
          ...
          <textarea
            ...
            placeholder="Payment terms, bank wire details..."
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <Button type="submit" disabled={isSubmitting}>
            {isSubmitting ? 'Creating...' : 'Create Customer Invoice'}
          </Button>
        </div>
      </form>
    </div>
