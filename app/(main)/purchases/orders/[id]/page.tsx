...
export default function PurchaseOrderDetailPage() {
  ...
  const handleConvertToBill = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`/api/purchase-orders/${id}/convert-to-bill`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      if (res.ok) {
        router.push(`/purchases/bills/${data.bill.id}`);
      } else {
        alert(data.message || 'Failed to convert to bill');
      }
    } catch {
      alert('Network error while converting purchase order');
    } finally {
  ...
  const handleUpdateStatus = async (newStatus: string) => {
    try {
      ...
      if (res.ok) {
        ...
        setToastMessage(`Purchase order marked as ${newStatus}`);
      }
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <div className="max-w-[1100px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-[24px] font-semibold text-[#111827]">{order.po_number}</h1>
              <span className={`inline-block px-2.5 py-0.5 text-[12px] font-semibold rounded-full border ${STATUS_COLORS[order.status]}`}>
                {order.status}
              </span>
            </div>
            <p className="text-[14px] text-[#667085] mt-1">Vendor: <span className="text-[#111827] font-medium">{order.vendor_name}</span></p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {order.status !== 'Cancelled' && (
              ...
              >
                {isConverting ? 'Creating Bill...' : 'Convert to Vendor Bill'}
              </Button>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-enterprise p-4">
          <span className="text-[12px] text-[#667085]">Expected Delivery</span>
          <p className="text-[15px] font-semibold text-[#111827] mt-0.5">
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-enterprise overflow-hidden mb-6 shadow-xs">
        <div className="px-5 py-3 border-b border-[#E5E7EB] bg-[#F7F8FA]">
          <h2 className="text-[14px] font-semibold text-[#111827]">Procured Items</h2>
        </div>
