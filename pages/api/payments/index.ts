...
async function handleGet(req: AuthenticatedRequest, res: NextApiResponse) {
  ...
  if (!dbOk) {
    ...
    if (limit && !isNaN(Number(limit))) {
      payments = payments.slice(0, parseInt(limit as string, 10));
    }
  ...
  try {
    ...
    let stored: any[] = getStoredPayments(type as string | undefined);
    ...
    return res.status(200).json({ payments: stored, source: 'fallback' });
  }
}
...
async function handleCreate(req: AuthenticatedRequest, res: NextApiResponse) {
  ...
  try {
    ...
    try {
      ...
      // Check partner_id UUID validity
      const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
      const validPartnerId = effectivePartnerId && uuidRegex.test(effectivePartnerId) ? effectivePartnerId : null;
